module Games::SurveysHelper
  ...
  def new_survey
    new_games_survey_path
  end

  def edit_survey(resource)
    edit_games_survey_path(resource)
  end

  def survey_scope(resource)
    if action_name =~ /new|create/
      games_surveys_path(resource)
    elsif action_name =~ /edit|update/
      games_survey_path(resource)
    end
  end

  def new_attempt
    new_games_attempt_path
  end

  def attempt_scope(resource)
    if action_name =~ /new|create/
     games_attempts_path(resource)
    elsif action_name =~ /edit|update/
      games_attempt_path(resource)
    end
  end
